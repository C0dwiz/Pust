...
class Module:
    """Base class for all Heroku modules"""
    ...
    def heroku_commands(self, value: Any) -> None:
        """Prevent setting commands directly"""
    ...
    def heroku_inline_handlers(self, value: Any) -> None:
        """Prevent setting inline_handlers directly"""
    ...
    def heroku_callback_handlers(self, value: Any) -> None:
        """Prevent setting callback_handlers directly"""
    ...
    def heroku_watchers(self, value: Any) -> None:
        """Prevent setting watchers directly"""
    ...
    async def _decline(
        ...
        declined = self._db.get("heroku.main", "declined_joins", [])
    ...
    async def request_join(
        ...
        declined_joins = self._db.get("heroku.main", "declined_joins", [])
    ...
    async def import_lib(
        ...
        *,
        ...
        if re.search(r"# ?scope: ?heroku_min", code):
            match = re.search(r"# ?scope: ?heroku_min ((\d+\.){2}\d+)", code)
            if match:
                ver = tuple(map(int, match.group(1).split(".")))
                if version.__version__ < ver:
                    _raise(
                        RuntimeError(
                            f"Library requires Heroku version "
                            f"{'{}.{}.{}'.format(*ver)}+"
                        )
                    )

        module_name = f"heroku.libraries.{url.replace('%', '%%').replace('.', '%d')}"
