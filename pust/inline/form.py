...
class Form(InlineUnit):
    async def form(
        ...
        reply_markup: typing.Optional[PustReplyMarkup] = None,
        *,
        force_me: bool = False,
        ...
        :param manual_security: By default, Pust will try to inherit inline buttons security from the caller (command)
                                If you want to avoid this, pass `manual_security=True`
        :param disable_security: By default, Pust will try to inherit inline buttons security from the caller (command)
        ...
        with contextlib.suppress(AttributeError):
            _Pust_client_id_logging_tag = copy.copy(self._client.tg_id)  # noqa: F841
    ...
    async def _form_inline_handler(self, inline_query: InlineQuery):
        """Handle inline queries for forms"""
            ...
            for unit in self._units.copy().values():
                for button in utils.array_sum(unit.get("buttons", [])):
                    if (
                        ...
                        and "input" in button
                        and button["_switch_query"] == first_word
        ...
        if query not in self._units or self._units[query]["type"] != "form":
            logger.debug(f"Form not found for query: {query}")

            if query in self._error_events:
                self._error_events[query] = Exception(
                    f"Form not found for unit_id: {query}"
                )
                self._error_events[query].set()
            return
        ...
        try:
            if "photo" in form:
                ...
                result = InlineQueryResultAudio(
                    ...
                    caption=form.get("text"),
                    parse_mode="HTML",
                    title=form["audio"].get("title", "Pust"),
                ...
                result = InlineQueryResultArticle(
                    ...
                    reply_markup=self.generate_markup(query),
                )
                await inline_query.answer([result], cache_time=0)

        except Exception as e:
            logger.error(f"Error in form handler: {e}")
