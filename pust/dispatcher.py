...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.Pust_me.id
        self._cached_usernames = [
            (
                self._client.Pust_me.username.lower()
                if self._client.Pust_me.username
                else str(self._client.Pust_me.id)
            )
        ]
