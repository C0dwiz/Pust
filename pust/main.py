...
from Pusttl.network.connection import (
    ConnectionTcpFull,
    ConnectionTcpMTProxyRandomizedIntermediate,
)
from Pusttl.password import compute_check
from Pusttl.sessions import MemorySession, SQLiteSession
...
class Pust:
    ...
    def _read_sessions(self) -> None:
        ...
        for filename in os.listdir(BASE_DIR):
            if (filename.startswith("Pust-") or filename.startswith("hikka-")) and filename.endswith(".session"):
    ...
    async def _badge(self, client: CustomTelegramClient) -> None:
        ...
        try:
            ...
            caption = (
                "🪐 <b>Pust {} started!</b>\n\n"
                "⚙ <b>GitHub commit SHA: <a href='https://github.com/coddrago/Pust/commit/{}'>{}</a></b>\n"
                "🔎 <b>Update status: {}</b>\n"
                "<b>{}</b>".format(
                    ".".join(map(str, __version__)),
                    build,
...
Pusttl.extensions.html.CUSTOM_EMOJIS = not get_config_key("disable_custom_emojis")
