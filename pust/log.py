"""Main logging module for Pust Userbot"""
...
def _patched_getlines(filename: str, module_globals=None) -> list[str]:
    ...
    source code of Pust modules properly. This is needed for
    interactive line debugger in werkzeug web debugger.
    """
    try:
        if filename.startswith("<") and filename.endswith(">"):
            module_name = filename[1:-1].split(maxsplit=1)[-1]
            if module_name.startswith("Pust.modules") and module_name in sys.modules:
...
class PustException:
    ...
    def __init__(
        ...
        full_stack: str,
        sysinfo: Optional[
            tuple[object, Exception, traceback.TracebackException]
        ] = None,
    ):
    ...
    def from_exc_info(
        ...
        """Create PustException from exception info"""
...
class TelegramLogsHandler(logging.Handler):
    ...
    def __init__(self, targets: Iterable[logging.Handler], capacity: int):
        ...
        self._tg_buffer: list[tuple[Union[str, PustException], Optional[int]]] = []
    ...
    def dumps(
        ...
        for record in self.buffer + self.handled_buffer:
            if record.levelno < level:
                continue
            if record.Pust_caller and client_id != record.Pust_caller:
                continue
    ...
    async def _prepare_and_send_logs(self) -> None:
        ...
        for client_id, module in self._modules.items():
            ...
            if client_logs:
                ...
                self._queue[client_id] = list(
                    utils.chunks(log_text, self._TG_MESSAGE_LIMIT)
                )

        exception_tasks = []
        for client_id, module in self._modules.items():
            for item in self._tg_buffer:
                if isinstance(item[0], PustException) and (
                    not item[1] or item[1] == client_id or self.force_send_all
                ):
                    exception = item[0]
                    if exception.sysinfo and isinstance(
                        exception.sysinfo[1], INTERNET_ERRORS
                    ):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        try:
            ...
            for frame_info in inspect.stack():
                ...
                candidate = frame_locals.get("_Pust_client_id_logging_tag")
                if isinstance(candidate, int):
                    caller_id = candidate
                    break

            record.Pust_caller = caller_id
        except Exception as e:
            logging.debug("Failed to determine caller: %s", e)
            record.Pust_caller = None
    ...
    def _process_telegram_log(
        self, record: logging.LogRecord, caller_id: Optional[int]
    ) -> None:
        ...
        if record.exc_info:
            ...
            exception = PustException.from_exc_info(
                *record.exc_info,
    ...
    def _dispatch_to_targets(self, record: logging.LogRecord) -> None:
        ...
        try:
            ...
            self.handled_buffer = self.handled_buffer[
...
class NoFetchUpdatesFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        return "Failed to fetch updates" not in message and "Sleep" not in message
