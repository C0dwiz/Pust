...
class PustBackupMod(loader.Module):
    ...
    async def client_ready(self):
        ...
        self._backup_channel, _ = await utils.asset_channel(
            ...
            silent=True,
            archive=True,
            avatar="https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/Pust/Pust_backups.png",
            _folder="Pust",
            invite_bot=True,
        )
    ...
    async def handler(self):
        try:
                ...
                reply_markup=self.inline.generate_markup(
                    [
                        [
                            {
                                "text": "↪️ Restore this",
                                "data": "Pust/backupall/restore/confirm",
                            }
                        ]
                    ]
                ),
            ...
            logger.exception("PustBackup failed")
            await asyncio.sleep(60)

    @loader.callback_handler()
    async def restore(self, call: BotInlineCall):
        if not call.data.startswith("Pust/backupall/restore"):
            return

        if call.data == "Pust/backupall/restore/confirm":
            await utils.answer(
                call,
                "❓ <b>Are you sure?</b>",
                reply_markup={
                    "text": "✅ Yes",
                    "data": "Pust/backupall/restore",
                },
            )
            return
        ...
        try:
            ...
            with zipfile.ZipFile(zipfile_bytes) as zf:
                ...
                with contextlib.suppress(KeyError):
                    db_data["Pust.inline"].pop("bot_token")
    ...
    async def restoredb(self, message: Message):
        ...
        with contextlib.suppress(KeyError):
            decoded_text["Pust.inline"].pop("bot_token")
