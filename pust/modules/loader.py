...
class LoaderMod(loader.Module):
    ...
    async def approve_internal(
        ...
        await call.edit(
            ...
            photo="https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/Pust/joined_jr.png",
        )
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?Pust_min", doc):
            ver = re.search(r"# ?scope: ?Pust_min ((?:\d+\.){2}\d+)", doc).group(1)
        ...
        try:
            try:
                ...
                async def inner_proxy():
                    ...
                    while True:
                        if hasattr(instance, "Pust_wait_channel_approve"):
                            if message:
                                (
                                    module,
                                    channel,
                                    reason,
                                ) = instance.Pust_wait_channel_approve
                                message = await utils.answer(
                                    message,
        ...
        try:
            developer_entity = await (
                ...
                if (
                    developer in self._client.Pust_entity_cache
                    and getattr(
