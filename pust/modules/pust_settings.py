...
@loader.tds
class PustSettingsMod(loader.Module):
    """Advanced settings for Pust Userbot"""

    strings = {"name": "PustSettings"}
    ...
    async def _uninstall(self, call: InlineCall):
        ...
        async for dialog in self._client.iter_dialogs(
            ...
            if (
                ...
                and (
                    ...
                    and dialog.name in {"Pust-logs", "silent-tags"}
                )
                or (
        ...
        if any(folder.title == "Pust" for folder in folders):
            folder_id = max(
                folders,
                key=lambda x: x.id,
            ).id
    ...
    @loader.command()
    async def uninstall_Pust(self, message: Message):
    ...
    async def remove_core_protection(self, message: Message):
        if self._db.get(main.__name__, "remove_core_protection"):
    ...
    async def enable_core_protection(self, message: Message):
        if not self._db.get(main.__name__, "remove_core_protection"):
    ...
    async def invokecmd(self, message: Message):
        ...
        if module == "core":
            if method == "flush_entity_cache":
                ...
                result = (
                    f"Dropped {len(self._client._Pust_fulluser_cache)} cache records"
                )
                ...
                result = f"Dropped {len(self._client._Pust_perms_cache)} cache records"
                self._client._Pust_perms_cache = {}
                ...
                result = (
                    f"Dropped {len(self._client._Pust_entity_cache)} entity cache"
                    " records\nDropped"
                    f" {len(self._client._Pust_fulluser_cache)} fulluser cache"
                    " records\nDropped"
                    f" {len(self._client._Pust_fullchannel_cache)} fullchannel cache"
                    " records\nDropped"
                ...
                self._client._Pust_fulluser_cache = {}
                self._client._Pust_fullchannel_cache = {}
                self._client.Pust_me = await self._client.get_me()
                ...
                result = (
                    "Entity cache:"
                    f" {len(self._client._Pust_entity_cache)} records\nFulluser cache:"
                    f" {len(self._client._Pust_fulluser_cache)} records\nFullchannel"
                    " cache:"
                    f" {len(self._client._Pust_fullchannel_cache)} records\nLoader"
