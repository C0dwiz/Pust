...
class UpdaterMod(loader.Module):
    """Updates itself, tracks latest Pust releases, and notifies you, if update is required"""
    ...
    async def update_call(self, call: InlineCall):
        ...
        if call.data not in {"Pust/update", "Pust/ignore_upd"}:
            return

        if call.data == "Pust/ignore_upd":
    ...
    async def client_ready(self):
        ...
        self._markup = lambda: self.inline.generate_markup(
            [
                {"text": self.strings("update"), "data": "Pust/update"},
                {"text": self.strings("ignore"), "data": "Pust/ignore_upd"},
            ]
        )
        ...
        if not self.config["autoupdate"] and not self.get("autoupdate", False):
            await self.inline.bot.send_photo(
                self.tg_id,
                photo="https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/Pust/unit_alpha.png",
    ...
    async def _add_folder(self):
        ...
        if any(getattr(folder, "title", None) == "Pust" for folder in folders.filters):
            return
        ...
        try:
            ...
            logger.critical(
                "Can't create Pust folder. Possible reasons are:\n"
