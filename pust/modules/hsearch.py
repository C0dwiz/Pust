...
class HSearch(loader.Module):
                ...
                try:
                    if self.config["tracking"]:
                        async with session.post(
                            ...
                            params={"user_id": self.uid, "lang": self.strings["lang"]},
    ...
    def _fmt_mod(
        ...
        if total > 1:
            info = (
                self.strings["result_query"].format(
                    idx=idx, total=total, query=utils.escape_html(query)
                )
                + info
            )
        elif query and not inline:
            info = (
                self.strings["result_single"].format(query=utils.escape_html(query))
                + info
            )
    ...
    def _fmt_cmds(self, cmds: List[Dict]) -> str:
        ...
        for cmd in cmds:
            ...
            if cmd.get("inline"):
                inline_cmds.append(
                    f"<code>@{self.inline.bot_username} {cmd_name}</code> {cmd_desc}"
                )
            else:
                regular_cmds.append(
                    f"<code>{self.get_prefix()}{cmd_name}</code> {cmd_desc}"
                )
            ...
            [
                {
                    ...
                    "callback": self._rate_cb,
                    "args": (install, "like", idx, mods, query),
                },
                {
                    "text": f"👎 {stats.get('dislikes', 0)}",
                    "callback": self._rate_cb,
    ...
    async def _nav_cb(self, call, idx: int, mods: List, query: str = ""):
        ...
        stats = (
            mod
            if all(k in mod for k in ["likes", "dislikes"])
            ...
            results.append(
                {
                    ...
                    "reply_markup": self._mk_btns(
                        mod.get("install", ""), stats, 0, None
                    ),
                }
            )
    ...
    async def hscmd(self, message):
        ...
        if len(mods) == 1:
            ...
            if (
                photo
                == "https://raw.githubusercontent.com/Fixyres/FHeta/refs/heads/main/assets/empty_pic.png"
            ):
                photo = None
        ...
        await self.inline.form(
            ...
            photo=photo,
            reply_markup=self._mk_btns(
                first_mod.get("install", ""),
                stats,
                0,
                mods if len(mods) > 1 else None,
                query,
            ),
        )
    ...
    async def _install_via_hsearch(self, message):
        ...
        try:
            for _ in range(5):
                ...
                is_loaded = any(
                    mod.__origin__ == link for mod in self.allmodules.modules
                )
